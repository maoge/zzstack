...
MAIN_CLASS=com.zzstack.paas.underlying.bench.DbBenchBootStrap
...
JAVA_OPTS="${JAVA_OPTS} -Ddisplay=pass-bench-db"
...
function check(){
	if [ -n "$PIDS" ]; then
	    echo "ERROR: The PAAS-BENCH-DB already started!"
	    echo "PID: $PIDS"
	    exit 127
	fi
}
...
function start(){
	check
	echo "JAVA_HOME:" $JAVA
	echo "Starting the PAAS-BENCH-DB ...\c"
	JAVA="java"
	...
	if [ $COUNT -gt 0 ]; then
	    ...
	    echo "ERROR!"
		echo " The PAAS-BENCH-DB is not start success!!!!"
		exit 127
	fi
}
...
function stop(){
	if [ -z "$PIDS" ]; then
        echo "ERROR: The PAAS-BENCH-DB does not started!"
        exit 127
	fi
	echo -e "Stopping the PAAS-BENCH-DB ...\c"
	for PID in $PIDS ; do

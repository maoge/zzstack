...
public class TDJniTools {
    
    private static Logger logger = LoggerFactory.getLogger(TDJniTools.class);
    
    public static void loadTDJniFile(Class<?> clazz) {
        String jniFileName = getTDJniFileName();
        if (!isFileExist(jniFileName)) {
            try {
                ...
                if (is == null) {
                    os.close();
                    logger.error("tdengine jni file:{} not exists ......", jniFileName);
                    return;
                }
    ...
    private static String getTDJniFileName() {
        if (OSTools.isLinux()) {
            return CONSTS.TD_JNI_LINUX;
        } else if (OSTools.isWindows()) {
            return CONSTS.TD_JNI_WINDOWS;
        } else {
            logger.error("tdengine jni only surpport window and linux");
            return "";
        }
    }
